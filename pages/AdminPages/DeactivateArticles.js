...
const DeactivateArticles = () => {
    ...
    useEffect(() => {
        ...
        const fetchRejectedArticles = async () => {
            try {
                const response = await axios.get('http://localhost:3001/api/rejectedArticle/getAll');
                ...
                const rejectedArticlesWithDetails = await Promise.all(response.data.map(async (rejectedArticle) => {
                    const rejectedArticleResponse = await axios.get(`http://localhost:3001/api/article/${rejectedArticle.articleId}`);
                                                    ...
                                                    <TableRow key={rejectedArticle.articleId}>
                                                        ...
                                                        <TableCell>
                                                            {(rejectedArticle.daysSinceDeactivation < 1) ? "" :
                                                                (
                                                                    <>
                                                                        <Button variant="contained" color="success" size="medium" sx={{ borderRadius: '4px', textTransform: 'capitalize' }} >Re-approve</Button>
